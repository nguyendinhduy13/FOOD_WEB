...
const MenuContainer = () => {
  ...
  return (
    <section className='w-full my-6' id="menu">
        ...
        <p className='text-2xl font-semibold captialize text-headingColor relative 
        ...
        {categories&&categories.map((category)=>(
            <motion.div 
            whileTap={{scale:0.75}}
            key={category.id} className={`group ${filter===category.urlParamName ? 'bg-cartNumBg':'bg-cardOverplay'} w-24 min-w-[94px] h-28 cursor-pointer rounded-lg shadow-lg drop-shadow-xl
            flex flex-col gap-3 items-center hover:bg-cartNumBg justify-center `}
